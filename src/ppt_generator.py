...
from pptx.enum.chart import XL_AXIS_CROSSES, XL_TICK_MARK, XL_TICK_LABEL_POSITION
    ...
    def add_bar_chart(
        ...
        if opposite_tick_labels:
            ...
            chart.category_axis.reverse_order = True
                        # Position category axis at the top
            chart.category_axis.crosses = XL_AXIS_CROSSES.MAXIMUM

        # Remove tick marks if desired
    ...
    def add_table(
        ...
        for i, col in enumerate(data.columns):
            ...
            for j in range(1, rows):
                ...
                for i, col in enumerate(data.columns):
                    data_cell = table.cell(j, i + 1)
                    data_cell.text = str(data.iloc[j - 1][col])
